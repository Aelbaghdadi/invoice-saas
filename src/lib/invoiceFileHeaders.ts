...
/**
 * Content-Type y Content-Disposition del original de una factura. La CSP
 * sandbox y nosniff de esa ruta van en next.config.ts: Next descarta las
 * cabeceras del route handler que ya trae la configuración.
 */
export function invoiceFileHeaders(
  ...
  return {
    ...
    "Content-Disposition": `attachment; filename="factura_${invoiceId}${extension}"`,
  };
}
